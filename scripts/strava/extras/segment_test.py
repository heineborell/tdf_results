...
for i, elmt in enumerate(ordered_list_mine):
    if i == 0:
...
longest_seg, max_cov = segment_picker(ordered_list)
...
(
    [
        print(
            ...
            ordered_list[index]["end_points"][1] - ordered_list[index]["end_points"][0],
        )
...
gdf_list = []
for i, df in enumerate(df_segment):
...
providers = [cx.providers.CartoDB.Voyager, cx.providers.OpenStreetMap]
## Plot the route with a background map
# fig, ax = plt.subplots(figsize=(10, 8))
# gdf.plot(ax=ax, color="blue", linewidth=2)
## ctx.add_basemap(ax, source=providers[0], crs=gdf.crs)  # Add background map
# plt.show()
